package haina.ecommerce.util

import android.content.SharedPreferences
import haina.ecommerce.preference.SharedPreference

class Constants {

//    val preference: SharedPreference=SharedPreference()

    companion object{
        const val BASE_URL_NEWS = "https://newsapi.org/"
        const val BASE_URL_REGISTER = "http://192.168.0.115:8000/"
        const val BASE_URL_CURRENCY = "https://api.currencyfreaks.com/"
        const val API_NEWS = "357c683838a746109ba2b6f718d5455f"
        const val WEB_CLIENT_ID_FIREBASE = "1039590465685-1kdfumqufqj5r1cce102mjpueprp1fjq.apps.googleusercontent.com"
        const val WEB_CLIENT_SECRET_FIREBASE = "NnwHdEVbiSpO5JaY1qyp_oB2"
        const val APIKEY = "HainaAPP"
        const val APIKEY_CURRENCY = "960e475d7c3340c8bb54f12fd3fe9fbb"
    }

}